;;; ttn/pgtable.scm --- Abstract manipulation of PostgreSQL tables
...
;;   (pg-result-symbol res)

;;; Code:

(define-module (ttn pgtable)
  :use-module (ice-9 common-list)
  :use-module (database postgres)
  :use-module (ttn pgtype)
  :use-module (ttn display-table)
  :autoload (ttn echo) (echo-n)
  :autoload (ttn stringutils) (string-append/separator))
...
(define (string* . things)
  (with-output-to-string
    (lambda () (apply echo-n things))))
...
(define (validate-def def)
  (or (object-property def 'validated)
      (and (pair? def)
           ...
           (let ((type-name (def:type-name def)))
             (and (symbol? type-name)
                  (memq type-name (map dbcoltype:name *db-col-types*))))
  ...
  (let* ((pgdb (pgdb-connection db-name))
    ...
    (lambda (choice)
      (case choice
        ...
        ((defs) defs)
        ((types-list) (map dbcoltype:name *db-col-types*))
        ((pgdb) pgdb)
        ((drop) drop)
        ((create) create)
...
;;; symbolic wrapper around `pg-result-status'

(define (pg-result-symbol res)
  (let ((orig (pg-result-status res)))
    (cond ((symbol? orig) orig)
          ((number? orig)
           (or (assq-ref
